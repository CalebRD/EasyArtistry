...
class ScriptPostprocessingAutosizedCrop(scripts_postprocessing.ScriptPostprocessing):
    ...
    def ui(self):
        with ui_components.InputAccordion(False, label="Auto-sized crop") as enable:
            ...
            with gr.Row():
                mindim = gr.Slider(minimum=64, maximum=1920, step=8, label="Dimension lower bound", value=640, elem_id="postprocess_multicrop_mindim")
                maxdim = gr.Slider(minimum=64, maximum=1920, step=8, label="Dimension upper bound", value=1920, elem_id="postprocess_multicrop_maxdim")
            with gr.Row():
                minarea = gr.Slider(minimum=64 * 64, maximum=1920 * 1080, step=1, label="Area lower bound", value=1920 * 64, elem_id="postprocess_multicrop_minarea")
                maxarea = gr.Slider(minimum=64 * 64, maximum=1920 * 1080, step=1, label="Area upper bound", value=1920 * 1080, elem_id="postprocess_multicrop_maxarea")
            with gr.Row():
